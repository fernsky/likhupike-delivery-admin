...
export default function GenderSEO({
  ...
  const generateStructuredData = () => {
    ...
    const genderStats = overallSummary.map((item) => ({
      ...
      description: `${localizeNumber(item.population.toLocaleString(), "ne")} ${item.genderName} घरमूली लिखु पिके गाउँपालिकामा रहेका छन् (कुल घरमूलीको ${localizeNumber(((item.population / totalPopulation) * 100).toFixed(2), "ne")}%)`,
    }));

    return {
      "@context": "https://schema.org",
      "@type": "Dataset",
      name: "Household Head Gender Distribution in Khajura Rural Municipality (लिखु पिके गाउँपालिका)",
      ...
      url: "https://digital.likhupikemun.gov.np/profile/demographics/ward-wise-househead-gender",
      creator: {
        "@type": "Organization",
