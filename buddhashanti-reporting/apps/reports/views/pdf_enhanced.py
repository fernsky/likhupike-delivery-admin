...
class PDFGeneratorMixin:
    ...
    def generate_pdf_with_reportlab(self, template_name, context, filename):
        ...
        # Build content
        content = []
        content.append(Paragraph("लिखु पिके गाउँपालिका - पूर्ण प्रतिवेदन", styles["Title"]))
...
class GenerateFullPDFView(PDFGeneratorMixin, TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        municipality_name_english = "likhupike Rural Municipality"
        ...
        filename = (
            f"likhupike_digital_profile_report_{timezone.now().strftime('%Y%m%d')}.pdf"
        )
...
class GenerateCategoryPDFView(PDFGeneratorMixin, TemplateView):
    def get(self, request, slug, *args, **kwargs):
        ...
        # Municipality name - make dynamic
        municipality_name = "लिखु पिके गाउँपालिका"
        municipality_name_english = "likhupikeRural Municipality"
        ...
        filename = (
            f"likhupike_{category.slug}_report_{timezone.now().strftime('%Y%m%d')}.pdf"
        )
        return self.generate_pdf_with_weasyprint(
...
class GenerateSectionPDFView(PDFGeneratorMixin, TemplateView):
    def get(self, request, category_slug, section_slug, *args, **kwargs):
        ...
        filename = f"likhupike_{category.slug}_{section.slug}_{timezone.now().strftime('%Y%m%d')}.pdf"
        return self.generate_pdf_with_weasyprint(
